    ...
    if(Option_vagas_valor == 'Livre'){
       ...
       let text;
        if (confirm("Deseja Apagar o Status da Vaga? \n Pressione o button!") == true) {
            ...
            cpf_valor = ''; //esqueci
            entrada_valor = 0;
            resetaPagina(); // qlqr coisa tirar6 
        } else {
            listarUsuarios(1);
...
function mostraAlerta(elemento)
    ...
    $('#search-input').on('keyup', function() {
        ...
        $('#my-table tbody tr').each(function() {
          var N_vaga_veiculo = $(this).find('td:nth-child(1)').text().toLowerCase();
          var N_vaga_pesquisa = $(this).find('td:nth-child(2)').text().toLowerCase();
          var nome_vaga_pesquisa = $(this).find('td:nth-child(4)').text().toLowerCase();
          var placa_vaga_pesquisa = $(this).find('td:nth-child(5)').text().toLowerCase();
          var status_vagas_pesquisa = $(this).find('td:nth-child(10)').text().toLowerCase();
