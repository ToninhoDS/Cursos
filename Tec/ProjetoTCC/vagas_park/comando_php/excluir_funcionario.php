...
$nm_origem ='LISTA DE FUNCIONARIO';
   ...
   $nm_descricao_acao ='Ação foi para DELETAR CADASTRO DO FUNCIONARIO ';
   $nm_origem ='LISTA DE FUNCIONARIO';
...
if (!empty($id)){
    ...
    $query_usuario ="DELETE FROM tb_funcionario WHERE cd_funcionario=:id";
    $result_usuario = $conn->prepare($query_usuario);
    $result_usuario->bindParam(':id',$id);
    
     // avalidar se foi registrado no banco de dados com sucesso
    if($result_usuario->execute()){
        $retorna = ['erro' => false, 'msg' => "<div class='alert alert-success' role='alert'>Excluido o Funcionario com Sucesso!</div>"];

    }else{
        $retorna = ['erro' => false, 'msg' => "<div class='alert alert-danger' role='alert'>Erro: Não Excluiu o Funcionario</div>"];
