...
if (!empty($pagina)) {
    $qnt_result_pg = $qt_tabela_vagas_detalhes; 
    $inicio = ($pagina * $qnt_result_pg) - $qnt_result_pg;
    $query_usuarios = "SELECT  cd_status_vagas, cd_numero_vaga, nm_nome, img_icon, dt_entrada, sg_placa, cd_cpf, nm_status FROM tb_status_vagas ORDER BY nm_status  DESC LIMIT $inicio, $qnt_result_pg";
...
while ($row_usuario = $result_usuarios->fetch(PDO::FETCH_ASSOC)) {
    ...
    $diff_hours -= 1;
    $diff_hours = abs($diff_hours); // tirar uma hora 
    if ($diff_minutes >= 60) {
