...
if($cargo == 'Funcionario'){
    //condição WHERE se for funcionario ou gerente
    $where_cd_ = 'f.cd_funcionario';
   $muda_cargo = 'none';
}else{
    $where_cd_ = 'f.cd_gerente';
   $muda_cargo = null;
}
// raiz do diretorio
...
if (!empty($pagina)) {
    ...
    $query_usuarios = "SELECT g.nm_gerente, f.cd_funcionario, f.nm_nome, f.nm_cargo, f.nm_sexo, f.cd_cpf, f.cd_credencial, f.cd_email_funcionario, f.img_imagem, f.cd_telefone, g.cd_gerente
    FROM tb_gerente as g
    JOIN tb_funcionario as f
    ON   f.cd_gerente = g.cd_gerente
    WHERE $where_cd_ = $id_Login ORDER BY f.cd_funcionario DESC LIMIT $inicio, $qnt_result_pg";
    ...
    if (($result_funcionario) and ($result_funcionario->rowCount() >= 0)) {
        ...
        while ($row_usuario = $result_funcionario->fetch(PDO::FETCH_ASSOC)) {
            ...
            <td >
                 <button style='display:$muda_cargo;' class='btn btn-info btn-sm me-1'  id='botao_visualizar$cd_funcionario' value='$cd_funcionario'name='id_visualizar$cd_funcionario' onclick='visualizar($cd_funcionario)'>Informações</button>
                 <button style='display:$muda_cargo;' class='btn btn-danger btn-sm me-1'  id='botao_delete_funcionario$cd_funcionario' value='$cd_funcionario'name='id_delete_funcionario$cd_funcionario' onclick='delete_funcionario($cd_funcionario)'>Delete</button>
             </td>
             </td>
        </tr>";
        }
        ...
        Não consta Cadastro de Prestador de  Serviço!
      </div>"];
    }
} else {
    $retorna = ['status' => false, 'msg' => "<div class='alert alert-danger' role='alert'>
